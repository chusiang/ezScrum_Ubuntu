...
function showByDate() {
	...
	document.location.href = "<html:rewrite action="/showFlowDiagram" />?startDate=" + startdate.value + "&endDate=" + enddate.value;
}
...
function refreshPercentImage(imageSrc)
{
	img = document.getElementById("percentDiagramChartImage");
	img.src = imageSrc+"?"+Math.random();
}
...
<BODY onload="refreshImage('${KanbanReport.flowDiagramChartPath}'); refreshPercentImage('${PercentReport.percentDiagramChartPath}');">
		...
		<table width="95%" align="center" cellpadding="2" cellspacing="0"
			...
			<tr>
				<td colspan="2" class="TaskBoardReportBody"  align="center">
					<img id="flowDiagramChartImage" src="${KanbanReport.flowDiagramChartPath}" class="LinkBorder"/>
					<img id="percentDiagramChartImage" src="${PercentReport.percentDiagramChartPath}" class="LinkBorder"/>
				</td>
			</tr>
			...
			<tr>
				<td class="TBReportSuccessHead" style="font-size: medium" width="20%" align="center">Average Cycle Time</td>
				<td class="TBReportSuccessHead" style="font-size: medium" align="center">${KanbanReport.averageCycleTime}</td>
			</tr>
		</table>
		</div>
		<br />
