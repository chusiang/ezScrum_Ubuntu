...
ezScrum.AttachFileForm = Ext.extend(Ext.form.FormPanel, {
	...
	submit : function()
	{
		if(this.getForm().isValid()){
			...
			myMask.show();
			var obj = this;
			var readUrl = this.url + "?issueID=" + this.issueId + "&projectName=" + this.projectName;
			var form = this.getForm().submit({
				url:readUrl,
				params:{entryPoint: "CustomIssue"},
...
ezScrum.AttachFileWidget = Ext.extend(Ext.Window, {
	...
	attachFile1:function(id){
		this.attachForm.reset();
		this.attachForm.issueId = id; 
		this.show();
	},
	attachFile:function(id, projectName){
		this.attachForm.reset();
		this.attachForm.issueId = id; 
