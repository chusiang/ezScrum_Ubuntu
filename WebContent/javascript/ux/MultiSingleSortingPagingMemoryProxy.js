 ...
 *          </p>
 */
Ext.ux.data.MultiSingleSortingPagingMemoryProxy = Ext.extend(
		Ext.data.MemoryProxy, {
			constructor : function(data, filter) {
				Ext.ux.data.MultiSingleSortingPagingMemoryProxy.superclass.constructor
						.call(this);
				this.data = data;
				this.filter = filter;
				this.reload = false;
			...
			doRequest : function(action, rs, params, reader, callback, scope,
					options) {
				params = params || {};
				var result = {
					success : true,
				...
				/*
				 * 1.欄位的Filter，依照每個欄位所勾選或設定的Filter條件
				 * Ex.Tag欄位勾選"Bug"，那麼就只會顯示欄位內含Bug這個標籤的資料出來
				 * 
				 * 2.依照使用者輸入的文字進行搜尋過濾
				 * Ex.使用者在Search的Field輸入"使用者",那麼就找出所有Name有包含使用者的Story
				 */
				// result.records =
				// result.records.filter(this.getFilter());
				var fun = this.getFilter();
				var tempRecord = new Array();
				for (var i = 0; i < result.records.length; i++) {
					if (fun(result.records[i])
							&& this.searchFilter(this.SearchComboBoxValue,result.records[i]))
				...
				for (var i = 0, j = sorters.length; i < j; i++) {
					sortFns.push(this.createSortFunction(sorters[i].field,
							sorters[i].direction, fields));
				}
				var directionModifier = direction.toUpperCase() == "DESC"
						? -1
						: 1;
			...
			searchFilter : function(comboBoxValue,record) {
				if (this.SearchText != null) {
					if (comboBoxValue == "Story Name"){
						if (record.get('Name').search(this.SearchText) != -1)
							return true;
					}else if (comboBoxValue == "Notes"){
						if (record.get('Notes').search(this.SearchText) != -1)
							return true;
					}else if (comboBoxValue == "How To Demo"){
						if (record.get('HowToDemo').search(this.SearchText) != -1)
							return true;
					}else if (comboBoxValue == "Release"){
						if (record.get('Release').search(this.SearchText) != -1)
							return true;
					}else if (comboBoxValue == "Sprint ID"){
						if (record.get('Sprint').search(this.SearchText) != -1)
							return true;
//					}else if (comboBoxValue == null){	// 當comboBox沒有選擇時。
//						if (record.get('Name').search(this.SearchText) != -1)  //預設textField內的值，比對Name。
//							return true;
					}else
						return false;
				} else {
