...
var IssueTagMenu = new Ext.menu.Menu({
    ...
    setInitTagInfo: function(record) {
    	// reload store data
//    	IssueTagStore.reload();		// reload 會導致下面執行的勾勾不見，但是應該要有地方作 reload
    	if ( record !== undefined) {
...
ezScrum.window.StoryWindow = Ext.extend(ezScrum.layout.Window, {
    ...
    showTheWindow_Add	: function(panel, sprintID) {
    	...
    	this.StoryForm.getForm().setValues({ sprintId : sprintID });
        this.StoryForm.notifyPanel = panel;
...
/*
 * call method
 * 		1. showTheWindow_Add: function(panel, sprintID)
 * 		2. showTheWindow_Edit: function(panel, issuueID)
 * 
 * notify method
 * 		1. notify_CreateStory: function(success, response, record)
 * 		2. notify_EditStory: function(success, response, record)
 * 
 * shared with: 
 * 		1. ProductBacklog
 * 		2. SprintBacklog
