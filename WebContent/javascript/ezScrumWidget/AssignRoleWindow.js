...
var AssignRoleManagement_RoleColumnModel = new Ext.grid.ColumnModel([
	{ header: "Project", width: 150, dataIndex: 'Resource' },
...
var AssignRoleManagement_RoleCombo = new Ext.form.ComboBox({
    ...
    listeners: {
    	...
    	'select': function(combo) {
	   		// Check add user button disable
	        if ((AssignRoleManagement_UnAssignRoleCombo.getValue()!="") && (combo.getValue()!=""))
	        	Ext.getCmp("AssignRoleManagement_AddRoleBtn").setDisabled(false);
	        else
	        	Ext.getCmp("AssignRoleManagement_AddRoleBtn").setDisabled(true);
	   	}
	...
	initComponent:function() {
		var config = {
			items: [{
            		...
            		defaultType	: 'textfield',
            		items : [
            			AssignRoleManagement_UnAssignRoleCombo,           			
            			AssignRoleManagement_RoleCombo
                	],
                	buttons: [{
			    		...
			    		handler	: function() {
			    			if ((AssignRoleManagement_UnAssignRoleCombo.getValue()!="") && (AssignRoleManagement_RoleCombo.getValue()!="")) {
								...
								Ext.Ajax.request({
									scope	: this,
									url		: 'addUser.do',
									params	: { id: cid, resource: selectProject, operation: accessLevel },
									success	: function(response){ this.onUpdateSuccess(response); },
									failure	: function(response){ this.onUpdateFailure(response); }
								});		
		    				}
			    		}
			    	}]
            		...
            		items : [{
		                ...
		                cm		: new Ext.grid.ColumnModel([
	                      	{ header: "Project", width: 200, dataIndex: 'Resource' },
	                      	{ header: "Role", width: 100, dataIndex: 'Operation' }
	                    ]),
                	...
                	buttons: [{
			    		...
			    		handler	: function() {
			    			...
			    			Ext.Ajax.request({
			    				scope	: this,
								url		: 'removeUser.do',
								params	: { id: cid, resource: resource, operation: operation },
								success	: function(response){ this.onUpdateSuccess(response); },
								failure	: function(response){ this.onUpdateFailure(response); }
							});		
			    		}
	...
	loadStore		: function() {
		ManagementMainLoadMaskShow();
 		Ext.Ajax.request({
			...
			success	: function(response){ this.onLoadSuccess(response); },
			failure	: function(response){ this.onLoadFailure(response); }
		});
	},
	reset			: function() {
		this.getForm().reset();
...
ezScrum.window.AssignRoleWindow = Ext.extend(ezScrum.layout.Window, {
	...
	initComponent:function() {
		var config = {
			layout	: 'form',
			items	: [{ xtype:'AssignRoleManagement_AssignRoleForm', ref: 'AssignRoleManagement_AssignRoleForm_refID' }]
        }
		Ext.apply(this, Ext.apply(this.initialConfig, config));
		...
		this.AssignRoleManagement_AssignRoleForm_refID.on('LoadSuccess', function(obj, response, record){ this.fireEvent('LoadSuccess', this, obj, response, record); }, this);
		this.AssignRoleManagement_AssignRoleForm_refID.on('LoadFailure', function(obj, response){ this.fireEvent('LoadFailure', this, obj, response); }, this);
		this.AssignRoleManagement_AssignRoleForm_refID.on('UpdateSuccess', function(obj, response, record){ this.fireEvent('UpdateSuccess', this, obj, response, record); }, this);
		this.AssignRoleManagement_AssignRoleForm_refID.on('UpdateFailure', function(obj, response){ this.fireEvent('UpdateFailure', this, obj, response); }, this);
	},
