...
function loadTagList(record) {
	...
	var loadmask = new Ext.LoadMask(productBacklogTagMenu.menu
			.getEl(), {
		msg : "loading info..."
	});
	loadmask.show();
...
ezScrum.ProductBacklogGrid = Ext.extend(ezScrum.IssueGridPanel, {
    ...
    updateRecord : function(record) {
    	...
    	if(expand)
	    	ProductBacklogExpander.expandRow(index);
...
ezScrum.ProductBacklogPage = Ext.extend(Ext.Panel, {
	...
	initComponent : function() {	
		var tb = new ezScrum.productBacklog.TopToolbar({
			items: [
			 		{itemId:'ProductBacklog_excelStoryBtn', text:'Import / Export Story', icon:'images/excel.png', 
			 			menu: {
			 				items:[
			 				    { text:'Import Story', icon:'images/import.png', handler:function(){ importStoriesWidget.importFile(); } },
			 				    { text:'Export Story', icon:'images/export.png', handler:function(){ openURLWithCheckSession('exportStories.do'); } }
			 				]
			 			}
			 		},
			 		{itemId:'ProductBacklog_addStoryBtn', text:'Add Story', icon:'images/add3.png', handler: function(){ Ext.getCmp('productBacklogMasterPanel').AddStoryAction(); } },
			 		{itemId:'ProductBacklog_editStoryBtn', disabled:true, text:'Edit Story', icon:'images/edit.png', handler:function(){ Ext.getCmp('productBacklogMasterPanel').editStory(); } },
			 		{itemId:'ProductBacklog_deleteStoryBtn', disabled:true, text:'Delete Story', icon:'images/delete.png', handler: function(){ Ext.getCmp('productBacklogMasterPanel').deleteStory(); } },
			 		{itemId:'ProductBacklog_showHistoryBtn', disabled:true, text:'Show History', icon:'images/history.png', handler: function(){ Ext.getCmp('productBacklogMasterPanel').showHistory() } },
			 		{itemId:'ProductBacklog_manageTagBtn', text:'Manage Tag', icon:'images/magic-wand.png', handler:function(){ Manage_Tag_Window.show(); } },
			 		{itemId:'ProductBacklog_tagMenu', disabled:true, text:'Tag', icon:'images/folder.png', menu:{ items:[], listeners:{ itemclick: function(baseItem, e) { Ext.getCmp('productBacklogMasterPanel').itemClick(baseItem, e);	}, show: function(menu) { Ext.getCmp('productBacklogMasterPanel').showTagMenu(); } }}},
			 		{itemId:'ProductBacklog_attachFileBtn', disabled:true, text:'Attach File', icon:'images/paperclip.png', handler:function(){ Ext.getCmp('productBacklogMasterPanel').attachFile(); } },
			 		{itemId:'ProductBacklog_filterMenu', disabled:false, text:'Filter', icon:'images/Filter.png', hideOnClick: true,
			 			menu:{
			 				items:[
			 				    { tagId: 'Backlog_Filter', text: 'Backlogged', hideOnClick: true, icon:'images/Filter.png', handler: function() { LoadData_PB(filter_type_Backlog);	productBacklogFilterMenu.setText("Backlogged");	} },
			 				    { tagId: 'Detail_Filter', text: 'Detailed', hideOnClick: true, icon:'images/Filter.png', handler: function() { LoadData_PB(filter_type_Detail); productBacklogFilterMenu.setText("Detailed"); }	},
			 				    { tagId: 'Done_Filter', text: 'Done', hideOnClick: true, icon:'images/Filter.png', handler: function() { LoadData_PB(filter_type_Done); productBacklogFilterMenu.setText("Done"); }	},
			 				    { tagId: 'No_Filter', text: 'Default', hideOnClick: true, icon:'images/clear2.png',	
			 				    	handler: function() { 
			 				    	LoadData_PB(null); 
		...
		var bb = new Ext.Toolbar({
			items:[
				'->',
					{ text:'Reload', icon:'images/refresh.png', handler: function() { LoadData_PB(null); } },
					...
					{ text:'Expand All', icon:'images/folder_out.png', handler:function() { ProductBacklogExpander.expandAll(); } },
					{ text:'Collapse All', icon:'images/folder_into.png', handler:function() { ProductBacklogExpander.collapseAll(); } }
			]
		});
	...
	showHistory: function() {
		...
		if(record != null) {
			var id = record.data['Id'];
			IssueHistory_Window.showTheWindow(id);
		}
	},
	...
	itemClick: function(baseItem, e) {
		...
		if(baseItem.checked == false) {
			Ext.Ajax.request({
				...
				params : { storyId: issueID, tagId: baseItem.tagId },
				success : function(response) {
					var rs = jsonStoryReader.read(response);
					if( rs.totalRecords == 1 ) {
						Ext.getCmp('productBacklogMasterPanel').ProductBacklog_GridPanel_ID.updateRecord(rs.records[0]);
