...
var search_field = new Ext.form.TextField({
	...
	comboBoxValue	: '',
	disabled		: true,
//	comboBoxDefault	: searchFieldStore.getAt(0).data['dataValue'],
	initEvents : function()
	{
	...
	search : function(value,text){
		...
		if (text.length > 0) {
			ProductBacklogStore.load({params:{start:0, limit:pageSize}});
		} else {
			ProductBacklogStore.load({params:{start:0, limit:pageSize}});
...
function initButton_MultiSort(){
    ...
    }, false));// false to not allow change
	tbar_multisort.add(createSorterButton({
        text: 'Sprint',
        sortData: {
            field: 'Sprint',
...
function doSort() {
	ProductBacklogStore.load({params:{start:Ext.getCmp('productBacklogGridPanel').getBottomToolbar().cursor, limit:pageSize}});
};
...
function initShowDetail(){
	...
	ProductBacklogStore.load({params:{start: parseInt(firstData/pageSize) * pageSize, limit: pageSize}});
